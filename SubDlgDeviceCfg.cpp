...
void CSubDlgDeviceCfg::CurCfgUpdate()
{
    ...
    if (NET_ECMS_GetDevConfig(m_lLoginID, NET_EHOME_GET_DEVICE_CFG, &struCfg, sizeof(struCfg)))
    {
        ...
        m_strDeviceName = m_struDevCfg.sServerName;
...
void CSubDlgDeviceCfg::CurCfgSetup()
{
    ...
    memcpy(m_struDevCfg.sServerName, m_strDeviceName.GetBuffer(0), m_strDeviceName.GetLength());
    ...
    DWORD dwConvertLen = 0;
    //库里面有转换
    if (!A2UTF8((char*)m_struDevCfg.sServerName, (char*)m_struDevCfg.sServerName, MAX_DEVICE_NAME_LEN, &dwConvertLen))
    {
        MessageBox("ServerName'length must less than 32 byte");
        return;
    }
    if (!A2UTF8((char*)m_struDevCfg.sSerialNumber, (char*)m_struDevCfg.sSerialNumber, MAX_SERIALNO_LEN, &dwConvertLen))
    {
        MessageBox("sSerialNumber'length must less than 32 byte");
        return;
    }
