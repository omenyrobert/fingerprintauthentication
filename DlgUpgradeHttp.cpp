...
void CDlgUpgradeHttp::OnBnClickedOk()
{
    ...
    NET_CMS_ISAPI_CONFIG struISAPIXML = { 0 };
    struISAPIXML.dwSize = sizeof(NET_CMS_ISAPI_CONFIG);
    ...
    struISAPIXML.lpInBuffer = szInBuffer;
    struISAPIXML.dwInBufferSize = sizeof(szInBuffer);
    struISAPIXML.dwRecvTimeOut = 5 * 1000;

    struISAPIXML.lpRequestUrl = szCmd;
    struISAPIXML.dwRequestUrlLen = strlen(szCmd);


    char szOutBuffer[1500] = { 0 };
    struISAPIXML.lpOutBuffer = szOutBuffer;
    struISAPIXML.dwOutBufferSize = sizeof(szOutBuffer);

    if (NET_ECMS_ISAPI_Config(m_lLoginID, &struISAPIXML))
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_SUCC_T, 1, "NET_ECMS_ISAPI_Config");
        //MessageBox("Start upgrade successful");
    }
    else
    {
        g_pMainDlg->AddLog(m_iDeviceIndex, OPERATION_FAIL_T, 1, "NET_ECMS_ISAPI_Config");
    ...
    int pCycleTimer = SetTimer(CYCLE_GETPROGRESS_TIMER, 200, NULL);
...
void CDlgUpgradeHttp::OnBnClickedBtnStopUpgrade()
{
    ...
    char szOutBuffer[1500] = { 0 };
    struISAPIXML.lpOutBuffer = szOutBuffer;
    struISAPIXML.dwRequestUrlLen = sizeof(szOutBuffer);
