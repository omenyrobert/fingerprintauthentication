...
void CSubDlgAlarmCfg::CurCfgSetup()
{
    ...
    memcpy(m_struAlarmOutCfg.sAlarmOutName, m_strAlarmOutName.GetBuffer(0), m_strAlarmOutName.GetLength());
    m_struAlarmOutCfg.wAlarmOutDelay = (WORD)m_cmbAlarmOutDelay.GetCurSel();

    //需要将字符串字段转换成UTF-8
    A2UTF8((char*)m_struAlarmOutCfg.sAlarmOutName, (char*)m_struAlarmOutCfg.sAlarmOutName, NAME_LEN, &dwConvertLen);
