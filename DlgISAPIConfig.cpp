...
void CDlgISAPIConfig::OnBnClickedBtnCommand()
{
    // TODO:  在此添加控件通知处理程序代码
    UpdateData(TRUE);
    NET_CMS_ISAPI_CONFIG struISAPIJSON = { 0 };
    struISAPIJSON.dwSize = sizeof(struISAPIJSON);
    struISAPIJSON.lpRequestUrl = m_sUrl.GetBuffer(0);
    struISAPIJSON.dwRequestUrlLen = m_sUrl.GetLength();
    struISAPIJSON.lpInBuffer = m_sInput.GetBuffer(0);
    struISAPIJSON.dwInBufferSize = m_sInput.GetLength();
    char sOutput[1024 * 10] = { 0 };
    struISAPIJSON.lpOutBuffer = sOutput;
    struISAPIJSON.dwOutBufferSize = sizeof(sOutput);
    ...
    if (NET_ECMS_ISAPI_Config(m_lUserID, &struISAPIJSON))
    {
